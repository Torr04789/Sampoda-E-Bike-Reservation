...
$ret="SELECT * FROM ebikereservation where (:fromdate BETWEEN date(FromDate) and date(ToDate) || :todate BETWEEN date(FromDate) and date(ToDate) || date(FromDate) BETWEEN :fromdate and :todate) and VehicleId=:vhid";
...
$sql="INSERT INTO  ebikereservation(BookingNumber,userEmail,VehicleId,FromDate,ToDate,PickLoc,ReturnLoc,message,Status) VALUES(:bookingno,:useremail,:vhid,:fromdate,:todate,:pickloc,:returnloc,:message,:status)";
...
echo "<script type='text/javascript'> document.location = 'my-reservation.php'; </script>";
}
else 
{
...
<aside class="col-md-4">
    ...
    <div class="form-group">
        <input type="submit" class="btn" id="reserveNowBtn" name="submit" value="Reserve Now" style="font-size: 12px;" disabled>
...
<?php } else { ?>
    <a href="#loginform" class="btn btn-xs uppercase" data-toggle="modal" data-dismiss="modal" style="font-size: 12px;">Login For Reserve</a>
<?php } ?>
...
<script>
    // JavaScript to enable the reserve Now button based on the checkbox status
    ...
    confirmTermsBtn.addEventListener('click', function() {
        if (termsCheckbox.checked) {
            reserveNowBtn.disabled = false; // Enable reserve Now button
...
{ ?>      
        <div class="col-md-3 grid_listing">
          <div class="product-listing-m gray-bg">
            <div class="product-listing-img"> <a href="vehical-details1.php?vhid=<?php echo htmlentities($result->id);?>"><img src="admin/img/vehicleimages/<?php echo htmlentities($result->Vimage1);?>" class="img-responsive" alt="image" /> </a>
            </div>
