...
$eid=intval($_GET['eid']);
$status="2";
$sql = "UPDATE ebikebooking SET Status=:status WHERE  id=:eid";
...
$sql = "UPDATE ebikebooking SET Status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT ebikeusers.*,ebikebrands.BrandName,ebikevehicles.VehiclesTitle,ebikebooking.FromDate,ebikebooking.ToDate,ebikebooking.message,ebikebooking.VehicleId as vid,ebikebooking.Status,ebikebooking.PostingDate,ebikebooking.id,ebikebooking.BookingNumber,
DATEDIFF(ebikebooking.ToDate,ebikebooking.FromDate) as totalnodays,ebikevehicles.PricePerDay
									  from ebikebooking join ebikevehicles on ebikevehicles.id=ebikebooking.VehicleId join ebikeusers on ebikeusers.EmailId=ebikebooking.userEmail join ebikebrands on ebikevehicles.VehiclesBrand=ebikebrands.id where ebikebooking.id=:bid";
$query = $dbh -> prepare($sql);
