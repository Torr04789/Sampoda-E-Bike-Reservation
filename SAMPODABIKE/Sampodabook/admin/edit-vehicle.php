...
$sql="update ebikevehicles set VehiclesTitle=:vehicletitle,VehiclesBrand=:brand,VehiclesOverview=:vehicleoverview,PricePerDay=:priceperday,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,Driver=:driver,GPSLocation=:gpslocation,LeatherSeats=:leatherseats where id=:id ";
...
<head>
	...
	<title>E-Bike Rental Reservation System | Admin Edit Vehicle Info</title>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT ebikevehicles.*,ebikebrands.BrandName,ebikebrands.id as bid from ebikevehicles join ebikebrands on ebikebrands.id=ebikevehicles.VehiclesBrand where ebikevehicles.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<option value="Rental">Rental with Driver</option></select>
</div>

<label class="col-sm-1 control-label">Select Driver<span style="color:red">*</span></label>
<div class="col-sm-2">
<select class="selectpicker" name="driver" required>
<option value="<?php echo htmlentities($result->Driver);?>"> <?php echo htmlentities($result->Driver);?> </option>
...
<input type="checkbox" id="inlineCheckbox1" name="gpslocation" checked value="1">
<label for="inlineCheckbox3"> GPS Location </label>
</div>
<?php } else {?>
<div class="checkbox checkbox-inline">
<input type="checkbox" id="inlineCheckbox1" name="gpslocation" value="1">
<label for="inlineCheckbox3"> GPS Location </label>
</div>
<?php } ?>
</div>
