...
$ret="SELECT * FROM ebikerental where (:fromdate BETWEEN date(FromDate) and date(ToDate) || :todate BETWEEN date(FromDate) and date(ToDate) || date(FromDate) BETWEEN :fromdate and :todate) and VehicleId=:vhid";
...
$sql="INSERT INTO  ebikerental(BookingNumber,userEmail,VehicleId,FromDate,ToDate,PickLoc,ReturnLoc,message,Status) VALUES(:bookingno,:useremail,:vhid,:fromdate,:todate,:pickloc,:returnloc,:message,:status)";
...
if($lastInsertId)
{
echo "<script>alert('Rental successfull.');</script>";
echo "<script type='text/javascript'> document.location = 'my-rental.php'; </script>";
}
else 
{
...
<aside class="col-md-4">
    ...
    <div class="sidebar_widget">
        ...
        <form method="post">
            ...
            <div class="form-group">
                <label>Rental return date & time:</label>
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    <select class="form-control" id="pickup_station" name="pickloc" required>
                        ...
                        <option value="178 Aguinaldo Hwy Brgy 1, Bacoor, Calabarzon">178 Aguinaldo Hwy Brgy 1, Bacoor, Calabarzon</option>
                        <option value="178 Aguinaldo, 108-176 R-2, Bacoor, Calabarzon">178 Aguinaldo, 108-176 R-2, Bacoor, Calabarzon</option>
                ...
                <div>
                    ...
                    <label for="pickup_input">E-Bike delivered at your place</label>
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    <label for="return_input">E-Bike Returned to your desired location</label>
                    <div style="display: flex; align-items: center;">
    ...
    <div class="form-group">
        <input type="submit" class="btn" id="rentNowBtn" name="submit" value="Rent Now" style="font-size: 12px;" disabled>
...
<?php } else { ?>
    <a href="#loginform" class="btn btn-xs uppercase" data-toggle="modal" data-dismiss="modal" style="font-size: 12px;">Login For Rent</a>
<?php } ?>
...
<script>
    // JavaScript to enable the Rent Now button based on the checkbox status
    ...
    confirmTermsBtn.addEventListener('click', function() {
        if (termsCheckbox.checked) {
            rentNowBtn.disabled = false; // Enable Rent Now button
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyABgvJKOnvPVhTzMqswXeom1YIbxsa3ohQ&libraries=places&callback=loadMap"></script>
...
<script>
    ...
    // Initialize autocomplete for both pickup and return locations
    ...
    function initMap(inputFieldId, latLng) {
        ...
        document.getElementById('selectLocationBtn').addEventListener('click', function() {
            ...
            geocoder.geocode({ location: selectedLatLng }, function(results, status) {
                if (status === 'OK') {
                    if (results[0]) {
                        document.getElementById(inputFieldId).value = results[0].formatted_address;
                    } else {
    ...
    google.maps.event.addDomListener(window, 'load', function() {
        initializeAutocomplete('pickup_custom');
        initializeAutocomplete('return_custom');
    });

    // Show Rental summary in terms modal
    document.getElementById('termsModal').addEventListener('show.bs.modal', function() {
                ...
                // Confirm terms and proceed to Rent
...
<div class="similar_cars">
  ...
  <div class="row">
    ...
    if ($query->rowCount() > 0) {
      foreach ($results as $result) { ?>      
        <div class="col-md-3 grid_listing" draggable="true">
          <div class="product-listing-m gray-bg">
            <div class="product-listing-img">
              <a href="vehical-details2.php?vhid=<?php echo htmlentities($result->id);?>">
                <img src="admin/img/vehicleimages/<?php echo htmlentities($result->Vimage1);?>" class="img-responsive uniform-image" alt="image" />
              </a>
            </div>
            <div class="product-listing-content">
              ...
              <ul class="features_list">
                ...
                <li><i class="fa fa-train" aria-hidden="true"></i><?php echo htmlentities($result->FuelType);?></li>
              </ul>
            </div>
          </div>
        </div>
...
<script>
  $(document).ready(function() {
    $('.grid_listing').on('dragstart', function(event) {
      $(this).css('opacity', '0.5'); // Change opacity when dragging starts
    });

    $('.grid_listing').on('dragend', function(event) {
      $(this).css('opacity', '1'); // Reset opacity after dragging ends
    });
  });
</script>
